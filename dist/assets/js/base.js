//-----------------------------------------------------------------------
// Version:        2.9
// Template name:  Chatbit
// Item URL :      https://themeforest.net/item/Chatbit-bootstrap-4-based-mobile-ui-kit-template/25384264
// Author:         Bragher
...
// Service Workers
//-----------------------------------------------------------------------
if (Chatbit.PWA.enable && 'serviceWorker' in navigator) {
...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("ChatbitAddHomeStatus");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("ChatbitAddHomeStatus", 1)
...
//-----------------------------------------------------------------------
// Dark Mode Detection
var checkDarkModeStatus = localStorage.getItem("ChatbitDarkMode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode-active");

// Check if enable as default
if (Chatbit.Dark_Mode.default) {
    pageBody.classList.add("dark-mode-active");
}

// Night Mode
if (Chatbit.Dark_Mode.night_mode.enable) {
    var nightStart = Chatbit.Dark_Mode.night_mode.start_time;
    var nightEnd = Chatbit.Dark_Mode.night_mode.end_time;
...
if (Chatbit.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("ChatbitDarkMode");
        var bodyCheck = pageBody.classList.contains('dark-mode-active');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode-active");
            localStorage.setItem("ChatbitDarkMode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode-active")
            switchDarkModeCheck(true);
            localStorage.setItem("ChatbitDarkMode", "1");
        }
    })
})
//-----------------------------------------------------------------------
...
if (document.querySelector(".cookies-modal") === null) {
    ...
    var CookiesStatus = localStorage.getItem("ChatbitCookiesStatus");
    ...
    document.querySelectorAll(".accept-cookies").forEach(function (el) {
        el.addEventListener("click", function () {
            localStorage.setItem("ChatbitCookiesStatus", "1");
        })
    })
...
function testMode() {
    ...
    console.clear();
    console.log("%cChatbit (v" + Chatbit.version + ")", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #1E74FD; padding: 14px 70px; margin-bottom: 16px;")
    ...
    testModeMsg(Chatbit.PWA.enable, "PWA")
    testModeMsg(Chatbit.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(Chatbit.Dark_Mode.night_mode.enable, "Night mode (between " + Chatbit.Dark_Mode.night_mode.start_time + ":00 and " + Chatbit.Dark_Mode.night_mode.end_time + ":00)")
    testModeMsg(Chatbit.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(Chatbit.RTL.enable, "RTL")
    testModeMsg(Chatbit.Test.enable, "Test mode")
    testModeMsg(Chatbit.Test.alert, "Test mode alert")
